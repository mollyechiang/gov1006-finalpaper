...
first_b <- stan_glm(prop_3km ~ avg_pwr_log + mindistlake + mindistlake_sq + longitude + latitude + long_sq + lat_sq + long_lat + as.factor(ed_id), data=matchobj, refresh = 0)
summary(first_b)
  
  # F-statistic on instrument 
mod1_b <- stan_glm(prop_3km ~ longitude + latitude + long_sq + lat_sq + mindistlake + mindistlake_sq + long_lat + as.factor(ed_id), data=matchobj, refresh = 0) # Run one model without the instrument.
mod2_b <- stan_glm(prop_3km ~ avg_pwr_log + longitude + latitude + long_sq + lat_sq + mindistlake + mindistlake_sq + long_lat + as.factor(ed_id), data=matchobj, refresh = 0) # Run a second model with the instrument.

  
  # Column 2, Second Stage
summary(ivreg(chng_lib ~ prop_3km + mindistlake + mindistlake_sq + longitude + long_sq + latitude + lat_sq + long_lat + as.factor(ed_id) | avg_pwr_log + mindistlake + mindistlake_sq + longitude + long_sq + latitude + lat_sq + long_lat + as.factor(ed_id), data = matchobj), digits = 4)
  
stargazer(first_b, title = "Regression")

```
```{r}
mod_1x <- lm(perc_lib ~ prop + factor(master_id) + factor(year), data=divisions)
