...
fig3a <- ggplot(plot_prop_omit, aes(y = ATT, x = dist)) +
  geom_point() +
...
fig3b <- ggplot(plot_op_omit, aes(y = ATT, x = dist)) +
  geom_point() +
  geom_hline(aes(yintercept = 0), color = "black", linetype = "dashed") +
  geom_errorbar(aes(ymin = ci_low, ymax = ci_up), width = .1) +
...
```{r stan_glm table 1, include=FALSE}
...
mod_1 <- stan_glm(perc_lib ~ prop + Y2003 + Y2007, data = yx_diffed)
...
mod_2 <- stan_glm(perc_lib ~ op + Y2003 + Y2007, data = yx_diffed)
...
mod_5 <- stan_glm(perc_lib ~ prop + Y2003 + Y2007, data = yx_diffed)
...
mod_6 <-
  stan_glm(perc_lib ~ op + Y2003 + Y2007, data = yx_diffed) #change treatment variable
...
mod_9 <-
  stan_glm(perc_lib ~ prop + Y2003 + Y2007,
     data = yx_diffed,
     weights = divisions$weights_p) # add in weights
...
mod_10 <-
  stan_glm(perc_lib ~ op + Y2003 + Y2007,
     data = yx_diffed,
     weights = divisions$weights_o) # add in weights
...
```{r print results of table 1}

print(mod_1, digits = 3)

print(mod_2, digits = 3)

print(mod_3, digits = 3)

print(mod_4, digits = 3)

print(mod_5, digits = 3)

print(mod_6, digits = 3)

print(mod_7, digits = 3)

print(mod_8, digits = 3)

print(mod_9, digits = 3)

print(mod_10, digits = 3)

print(mod_11, digits = 3)

print(mod_12, digits = 3)
```
By running a Bayesian regression on their model, I was able to find that their test was more robust than they may have expected. Not only did the coefficients match up from the lm() and stan_glm() models, the median absolute deviation also matches with the clusters of standard deviation found earlier. I was unable to run the cluster function on the Bayesian models and am planning to explore this futher.

```{r stan_glm table 2, include=FALSE}
...
```{r stan_glm table 3, include=FALSE}
...
mod_1 <- stan_glm(perc_turnout ~ prop + Y2003 + Y2007, data = yx_diffed)
...
mod_2 <-
  stan_glm(perc_turnout ~ prop_3km + Y2003 + Y2007, data = yx_diffed)
...
mod_3 <- stan_glm(perc_turnout ~ op + Y2003 + Y2007, data = yx_diffed)
...
mod_4 <- stan_glm(perc_turnout ~ op_3km + Y2003 + Y2007, data = yx_diffed)
...
```{r stan_glm table 4, include=FALSE}
...
mod_1 <- stan_glm(perc_lib_fed ~ prop + Y2003 + Y2007, data = yx_diffed)
...
mod_2 <-
  stan_glm(perc_lib_fed ~ prop_3km + Y2003 + Y2007, data = yx_diffed)
...
mod_3 <- stan_glm(perc_lib_fed ~ op + Y2003 + Y2007, data = yx_diffed)
...
mod_4 <- stan_glm(perc_lib_fed ~ op_3km + Y2003 + Y2007, data = yx_diffed)
